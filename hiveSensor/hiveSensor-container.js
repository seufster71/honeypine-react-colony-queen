...
import * as actions from './hiveSensor-actions';
import fuLogger from '../../../core/common/fu-logger';
import HiveSensorView from '../../../memberView/colony/hiveSensor/hiveSensor-view';
import HiveSensorModifyView from '../../../memberView/colony/hiveSensor/hiveSensor-modify-view';
import BaseContainer from '../../../core/container/base-container';

function CCHiveSensorContainer({location,navigate}) {
	const itemState = useSelector((state) => state.cchivesensor);
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"CC_HIVESENSOR_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'CCHiveSensorContainer::onOption',msg:" code "+code});
		...
		switch(code) {
			case 'SENSORDATA': {
				newPath = newPath + "/cc-sensorData";
				navigate(newPath,{state:{parent:item,parentType:"HIVESENSOR"}});
				break;
			}
	...
	fuLogger.log({level:'TRACE',loc:'CCHiveSensorContainer::render',msg:"Hi there"});
	if (itemState.view == "MODIFY") {
		return (
			<HiveSensorModifyView
			itemState={itemState}
			appPrefs={appPrefs}
			onSave={onSave}
			onCancel={onCancel}
...
export default CCHiveSensorContainer;
