...
import OrganizationView from '../../../memberView/colony/organization/organization-view';
import OrganizationModifyView from '../../../memberView/colony/organization/organization-modify-view';
import BaseContainer from '../../../core/container/base-container';


function CCOrganizationContainer({location,navigate}) {
	const itemState = useSelector((state) => state.ccorganization);
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"CC_ORGANIZATION_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'CCOrganizationContainer::onOption',msg:" code "+code});
		...
		switch(code) {
			case 'HIVE': {
				newPath = newPath + "/cc-hive";
				navigate(newPath,{state:{parent:item,parentType:"ORGANIZATION"}});
				break;
			}
		}
	}

	fuLogger.log({level:'TRACE',loc:'CCOrganizationContainer::render',msg:"Hi there"});
	if (itemState.view == "MODIFY") {
		return (
			<OrganizationModifyView
			itemState={itemState}
			appPrefs={appPrefs}
			onSave={onSave}
			onCancel={onCancel}
...
export default CCOrganizationContainer;
