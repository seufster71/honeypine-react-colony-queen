...
import * as actions from './hive-actions';
import fuLogger from '../../../core/common/fu-logger';
import HiveView from '../../../memberView/colony/hive/hive-view';
import HiveModifyView from '../../../memberView/colony/hive/hive-modify-view';
import BaseContainer from '../../../core/container/base-container';

function CCHiveContainer({location,navigate}) {
	const itemState = useSelector((state) => state.cchive);
	...
	const onSave = () => {
		BaseContainer.onSave({state:itemState,actions:actions,dispatch:dispatch,appPrefs:appPrefs,form:"CC_HIVE_FORM"});
	}
	const closeModal = () => {
	...
	const onOption = (code,item) => {
		fuLogger.log({level:'TRACE',loc:'CCHiveContainer::onOption',msg:" code "+code});
		...
		switch(code) {
			case 'HIVESENSOR': {
				newPath = newPath + "/cc-hiveSensor";
				navigate(newPath,{state:{parent:item,parentType:"HIVE"}});
				break;
			}
		}
	}

	fuLogger.log({level:'TRACE',loc:'CCHiveContainer::render',msg:"Hi there"});
...
export default CCHiveContainer;
